    ...
    await fc.assert(
      fc.asyncProperty(textGen, async ({ error, after }) => {
        ...
        try {
          ...
          const correction = createCorrection(text, 0, error.length, correctionMap[error]);
    ...
    await fc.assert(
      fc.asyncProperty(testCases, async (testCase) => {
        ...
        try {
          ...
          const actualValue = textarea.value;
          const actualBefore = actualValue.substring(0, testCase.correction.start);
          expect(actualBefore).toBe(testCase.expectedBefore);

          // Property: Text after the corrected error should be preserved
          const suffixStart = testCase.correction.start + testCase.correction.corrected.length;
          const actualAfter = actualValue.substring(suffixStart);
          expect(actualAfter).toBe(testCase.expectedAfter);

          // Property: Other similar errors should remain unchanged
          expect(actualAfter).toContain(testCase.text.substring(testCase.correction.end));
        } finally {
          cleanup(textarea);
        }
      }),
    ...
    await fc.assert(
      fc.asyncProperty(textGen, async ({ before, error, after }) => {
        ...
        try {
          ...
          const expectedLength = before.length + correctionMap[error].length + after.length;
