    ...
    await fc.assert(
      fc.asyncProperty(textGen, async ({ text, errors }) => {
        ...
        try {
          ...
          const lengthDiff1 = errors[0].corrected.length - (errors[0].end - errors[0].start);
      ...
      fc.asyncProperty(
        ...
        async (testCase, before, after) => {
          ...
          try {
            ...
            expect(correction.error.end).toBe(before.length + testCase.original.length);
    ...
    await fc.assert(
      fc.asyncProperty(textGen, async ({ text, correction: corrData }) => {
        ...
        try {
          ...
          expect(textarea.value).toBe('I could have done alot and you could of helped');
