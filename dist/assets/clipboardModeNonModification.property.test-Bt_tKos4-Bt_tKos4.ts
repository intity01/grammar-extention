...
describe("Property 8: Clipboard Mode Non-Modification", () => {
    ...
    await fc.assert(
      fc.asyncProperty(textGen, async ({ before, error, after }) => {
        ...
        try {
          ...
          await requestClipboardCorrection(
            before + correctionMap[error] + after,
          );
    ...
    await fc.assert(
      fc.asyncProperty(textGen, async ({ before, error, after }) => {
        ...
        try {
          ...
          expect(
            actualValue.substring(before.length, before.length + error.length),
          ).toBe(originalError);

          // Check suffix is preserved
          expect(actualValue.substring(before.length + error.length)).toBe(
            originalAfter,
          );
        } finally {
          cleanup(textarea);
  ...
  it("should preserve cursor position when using clipboard mode", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        textGen,
        async ({ before, error, after, cursorOffset }) => {
          const text = before + error + after;
          const textarea = createTestTextarea(text);
      ...
      fc.asyncProperty(
        ...
        async (testCase, before, after) => {
          ...
          try {
            ...
            // Request clipboard correction (clipboard mode)
            await requestClipboardCorrection(
              before + testCase.corrected + after,
            );
