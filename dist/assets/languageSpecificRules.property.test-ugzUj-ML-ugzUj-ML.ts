...
describe("Property 11: Language-Specific Rule Application", () => {
  ...
  it("should apply only Thai rules to Thai text segments", async () => {
    ...
    const thaiTextGen = fc
      .array(thaiCharGen, { minLength: 10, maxLength: 50 })
    ...
    await fc.assert(
      fc.asyncProperty(thaiTextGen, async (text) => {
        ...
        for (const segment of segments) {
          ...
          const matches = applyRulesToSegment(
            segment.text,
            segment.language,
            compiledRules,
          );
  ...
  it("should apply only English rules to English text segments", async () => {
    ...
    const englishTextGen = fc
      .array(englishCharGen, { minLength: 10, maxLength: 50 })
      .map((chars) => chars.join(""));
  ...
  it("should apply only Japanese rules to Japanese text segments", async () => {
    ...
    const japaneseTextGen = fc
      .array(japaneseCharGen, { minLength: 10, maxLength: 50 })
      .map((chars) => chars.join(""));
      ...
      fc.asyncProperty(mixedTextGen, async (text) => {
        ...
        for (const segment of segments) {
          ...
          for (const otherLang of otherLanguages) {
            const otherMatches = applyRulesToSegment(
              segment.text,
              otherLang,
              compiledRules,
            );
            ...
            expect(
              otherMatches.every((m) => m.ruleLanguage === otherLang),
            ).toBe(true);
          }
        }
      }),
  ...
  it("should verify rule language matches segment language for all matches", async () => {
    ...
    const mixedTextGen = fc
      .array(fc.oneof(thaiCharGen, englishCharGen, japaneseCharGen), {
        minLength: 20,
        maxLength: 100,
      })
      .map((chars) => chars.join(""));
    ...
    await fc.assert(
      fc.asyncProperty(mixedTextGen, async (text) => {
        ...
        for (const segment of segments) {
          ...
          for (const otherLang of otherLanguages) {
            const otherLangRules = compiledRules.filter(
              (r) => r.language === otherLang,
            );
  ...
  it("should apply correct rules when segments alternate between languages", async () => {
    ...
    const alternatingTextGen = fc
      ...
      .map((pairs) => {
        ...
        return pairs
          .map(([thai, english]) => thai.join("") + english.join(""))
          .join("");
      });
