...
async function testRuleMatcher() {
  ...
  const englishRules = readFileSync(
    join(__dirname, "../rules/english.json"),
    "utf-8",
  );
  ...
  // Load Thai rules
  console.log("\nLoading Thai rules...");
  const thaiRules = readFileSync(
    join(__dirname, "../rules/thai.json"),
    "utf-8",
  );
  ...
  const japaneseRules = readFileSync(
    join(__dirname, "../rules/japanese.json"),
    "utf-8",
  );
  ...
  if (englishErrors.length > 0) {
    console.log(
      `  - Error: "${englishErrors[0].original}" → "${englishErrors[0].correction}"`,
    );
  ...
  if (thaiErrors.length > 0) {
    console.log(
      `  - Error: "${thaiErrors[0].original}" → "${thaiErrors[0].correction}"`,
    );
  ...
  if (japaneseErrors.length > 0) {
    console.log(
      `  - Error: "${japaneseErrors[0].original}" → "${japaneseErrors[0].correction}"`,
    );
  ...
  if (multiErrors.length === 2) {
    console.log(
      `  - Error 1: "${multiErrors[0].original}" → "${multiErrors[0].correction}"`,
    );
    console.log(
      `  - Error 2: "${multiErrors[1].original}" → "${multiErrors[1].correction}"`,
    );
  ...
  if (posErrors.length > 0) {
    ...
    if (extracted === error.original) {
      ...
      console.log(
        `✗ Position mismatch: expected "${error.original}", got "${extracted}"`,
      );
    }
  }
