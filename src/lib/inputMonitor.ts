...
export class InputMonitor {
  ...
  observeField(element: InputElement): void {
    ...
    if (this.isContentEditable(element)) {
      ...
      element.addEventListener('input', this.handleInput.bind(this), { signal });
      element.addEventListener('focus', this.handleFocus.bind(this), { signal });
   ...
   */
  private isTextInput(element: HTMLElement): element is HTMLTextAreaElement | HTMLInputElement {
    return (
   ...
   */
  private getTextNodeAtPosition(element: HTMLElement, position: number): { node: Node; offset: number } | null {
    let currentPos = 0;
