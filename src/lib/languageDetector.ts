...
function isEnglishChar(char: string): boolean {
  const code = char.charCodeAt(0);
  return (
    (code >= UNICODE_RANGES.ENGLISH_UPPER.start && code <= UNICODE_RANGES.ENGLISH_UPPER.end) ||
    (code >= UNICODE_RANGES.ENGLISH_LOWER.start && code <= UNICODE_RANGES.ENGLISH_LOWER.end)
  );
}
