...
export function getMetrics(): PerformanceMetrics & {
  ...
  const avgTime = metrics.analysisCount > 0
    ? metrics.totalTime / metrics.analysisCount
    : 0;

  const recentAvg = metrics.recentTimes.length > 0
    ? metrics.recentTimes.reduce((a, b) => a + b, 0) / metrics.recentTimes.length
    : 0;

  const slowPercentage = metrics.analysisCount > 0
