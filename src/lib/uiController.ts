...
export function createRangeForError(
  ...
  function findTextNodeAtOffset(
    ...
    targetEnd: number,
    currentOffset = 0
  ): { startNode: Text | null; startOffset: number; endNode: Text | null; endOffset: number; found: boolean } {
...
export function preserveCursor(
  ...
  if (
    ...
    preserveCursorInInput(element, modificationStart, modificationEnd, newTextLength);
  } else if (element.isContentEditable) {
    preserveCursorInContentEditable(element, modificationStart, modificationEnd, newTextLength);
  }
}
...
function preserveCursorInContentEditable(
  ...
  const currentOffset = getOffsetInElement(element, range.startContainer, range.startOffset);
  ...
  } else if (currentOffset >= modificationStart && currentOffset <= modificationEnd) {
