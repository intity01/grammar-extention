 ...
 */
export function recordUsage(event: string, metadata?: Record<string, any>): void {
...
export function getErrorStats(context?: string): {
  ...
  const mostCommon = Object.entries(errorFrequency).sort((a, b) => b[1] - a[1])[0];
...
export function getHealthStatus(): {
  ...
  const slowOperations = performanceMetrics.filter((m) => m.duration > 100).length;
  if (slowOperations > totalOperations * 0.1) {
    issues.push(`Many slow operations: ${slowOperations} out of ${totalOperations}`);
  }
...
export function logHealthStatus(): void {
  ...
  console.log(`\nPerformance Metrics:`);
  console.log(`  - Average analysis time: ${perfStats.avgDuration.toFixed(2)}ms`);
