...
import { getSettings, updateSettings, getDefaultSettings } from '../lib/settings';
import { loadGrammarRules, compileGrammarRules, type CompiledGrammarRule } from '../lib/grammarRules';
import { hydrateDictionaries } from '../lib/dictionaryStorage';
import { initializeErrorMonitoring, logError } from '../lib/errorMonitoring';
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  try {
    ...
    logError(error as Error, { context: 'onInstalled' });
  }
});
...
async function sendToWorker(type: string, payload: any): Promise<any> {
  ...
  // Track loaded state
  if (type === 'LOAD_DICTIONARY' && result) {
    workerLoadedDictionaries.add(payload.language);
  }
  if (type === 'LOAD_RULES' && result) {
    workerLoadedRules.add(payload.language);
  }
...
function terminateWorker(): void {
  if (worker) {
    ...
    // Note: Keep workerLoadedDictionaries and workerLoadedRules for recovery
  }
}
...
async function analyzeTextViaWorker(text: string): Promise<AnalysisResult> {
  ...
  try {
    ...
    recordPerformance('worker_analyze', duration, {
    ...
    recordError(error as Error, 'analyzeTextViaWorker');

    // Attempt recovery: restart worker and retry once
    console.log('Attempting to recover by restarting worker...');
    recordUsage('worker_recovery_attempt');
    terminateWorker();

    try {
      // Reload dictionaries and rules that were previously loaded
      console.log('Reloading worker state...');
      for (const lang of workerLoadedDictionaries) {
        await sendToWorker('LOAD_DICTIONARY', { language: lang });
      }
      for (const lang of workerLoadedRules) {
        await sendToWorker('LOAD_RULES', { language: lang });
      }
      
      const result = await sendToWorker('ANALYZE', { text });
      console.log('Recovery successful');
      recordUsage('worker_recovery_success');
      
      // Record performance for recovery
      const duration = performance.now() - startTime;
      recordPerformance('worker_analyze_recovered', duration, {
        textLength: text.length,
        errorCount: result.errors?.length || 0,
      });
      ...
      recordError(retryError as Error, 'analyzeTextViaWorker_recovery');
      recordUsage('worker_recovery_failed');
      throw retryError;
    }
  }
}
 ...
 * Uses queue to handle concurrent requests
 */
async function writeToClipboard(text: string): Promise<void> {
  return new Promise((resolve, reject) => {
    clipboardQueue.push({ text, resolve, reject });
    processClipboardQueue();
  });
}

/**
 * Process clipboard queue sequentially
 */
async function processClipboardQueue(): Promise<void> {
  if (isProcessingClipboard || clipboardQueue.length === 0) {
    return;
  }

  isProcessingClipboard = true;
  const request = clipboardQueue.shift()!;
  const startTime = performance.now();
  ...
  try {
    ...
    // Record performance
    const duration = performance.now() - startTime;
    recordPerformance('clipboard_write', duration, {
      textLength: request.text.length,
    });
    recordUsage('clipboard_write_success');
    
    request.resolve();

    // Schedule cleanup after 30 seconds (only if queue is empty and no pending requests)
    setTimeout(async () => {
    ...
    recordError(error as Error, 'processClipboardQueue');
    recordUsage('clipboard_write_failed');
 ...
 */
async function loadAndCacheGrammarRules(language: Language): Promise<CompiledGrammarRule[]> {
  // Check cache first
  ...
  try {
    ...
    console.log(`Grammar rules loaded and cached for ${language}: ${compiledRules.length} rules`);
 ...
 */
async function getGrammarRules(language: Language): Promise<CompiledGrammarRule[]> {
    ...
    try {
      switch (message.type) {
        ...
        case 'GET_TELEMETRY': {
          ...
          const { getAllMetrics, getHealthStatus } = await import('../lib/telemetry');
          const metrics = getAllMetrics();
          const health = getHealthStatus();
          sendResponse({ success: true, metrics, health });
          break;
        }

        case 'CLEAR_TELEMETRY': {
          // Clear telemetry metrics
          const { clearMetrics } = await import('../lib/telemetry');
          clearMetrics();
