...
import { recordAnalysisTime } from '../lib/performanceMonitoring';
import { logError } from '../lib/errorMonitoring';
import { recordPerformance, recordUsage, recordError } from '../lib/telemetry';
import '../lib/telemetryHelpers'; // Load telemetry helpers for console access
    ...
    inputMonitor = new InputMonitor(
      {
        ...
        onFieldUnobserved: (element) => {
          // Clean up click handlers when field is unobserved
          removeErrorClickHandlers(element);
...
function establishKeepAliveConnection(): void {
  try {
    ...
    console.error('Grammar Checker: Failed to establish Keep-Alive connection:', error);
  }
}
...
function attachToInputFields(): void {
  ...
  const totalFields = textareas.length + inputs.length + contentEditables.length;
...
const elementClickHandlers = new WeakMap<InputElement, (e: Event) => void>();

/**
 * Handle text change events from Input Monitor
 * Sends text to Service Worker for analysis
 */
async function handleTextChange(text: string, element: InputElement): Promise<void> {
  ...
  try {
    // Record usage
    recordUsage('text_analyzed', { textLength: text.length });
    ...
    recordAnalysisTime(analysisTime, text.length);

    // Update performance metrics
    updatePerformanceMetrics(analysisTime);
    
    // Record telemetry
    recordPerformance('analyze', analysisTime, {
      textLength: text.length,
      errorCount: response.result?.errors?.length || 0,
    });
    ...
    logError(error as Error, { context: 'handleTextChange', textLength: text.length });
    recordError(error as Error, 'handleTextChange');
  }
}
...
function updatePerformanceMetrics(analysisTime: number): void {
  ...
  performanceMetrics.maxTime = Math.max(performanceMetrics.maxTime, analysisTime);
  performanceMetrics.minTime = Math.min(performanceMetrics.minTime, analysisTime);
  ...
  if (
    ...
    const avgTime = performanceMetrics.totalTime / performanceMetrics.analysisCount;
    const recentAvg = performanceMetrics.recentTimes.reduce((a, b) => a + b, 0) / 
                      performanceMetrics.recentTimes.length;
    const slowPercentage = (performanceMetrics.slowAnalysisCount / performanceMetrics.analysisCount) * 100;
...
export function getPerformanceMetrics() {
  const avgTime = performanceMetrics.analysisCount > 0
    ? performanceMetrics.totalTime / performanceMetrics.analysisCount
    : 0;
  
  const recentAvg = performanceMetrics.recentTimes.length > 0
    ? performanceMetrics.recentTimes.reduce((a, b) => a + b, 0) / performanceMetrics.recentTimes.length
    : 0;
  ...
  return {
    ...
    avgTime,
    recentAvg,
    minTime: performanceMetrics.minTime === Infinity ? 0 : performanceMetrics.minTime,
    maxTime: performanceMetrics.maxTime,
    slowAnalysisCount: performanceMetrics.slowAnalysisCount,
    slowPercentage: performanceMetrics.analysisCount > 0
      ? (performanceMetrics.slowAnalysisCount / performanceMetrics.analysisCount) * 100
      : 0,
  };
}
 ...
 */
function attachErrorClickHandlers(element: InputElement, _result: AnalysisResult): void {
  // Remove old handler if exists
  const oldHandler = elementClickHandlers.get(element);
  if (oldHandler) {
  ...
  const clickHandler = (event: Event) => {
    ...
    const clickedError = findErrorAtPosition(element, mouseEvent, analysisResult.errors);
  ...
  elementClickHandlers.set(element, clickHandler);
}

/**
 * Remove click handlers from an element
 */
function removeErrorClickHandlers(element: InputElement): void {
  const handler = elementClickHandlers.get(element);
  if (handler) {
    element.removeEventListener('click', handler);
    elementClickHandlers.delete(element);
  }
}
...
function showCorrectionPopup(
  ...
  applyButton.textContent = currentSettings?.correctionMode === 'clipboard' 
    ? 'Copy to Clipboard' 
    : 'Apply Correction';
...
export async function handleCorrectionAccepted(
  ...
  try {
    // Record usage
    recordUsage('correction_accepted', {
      mode: currentSettings.correctionMode,
      errorType: correction.error.type,
    });
    ...
    recordError(error as Error, 'handleCorrectionAccepted');
  }
}
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === 'sync' && changes.settings) {
    ...
    if (inputMonitor && newSettings.debounceDelay !== currentSettings?.debounceDelay) {
