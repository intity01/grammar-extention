...
describe("Property 2: Error Highlighting Completeness", () => {
  beforeEach(() => {
    ...
    Element.prototype.attachShadow = jest.fn(function (
      this: Element,
      init: ShadowRootInit,
    ) {
  ...
  const nonOverlappingErrorsGen = (textLength: number, maxErrors: number = 5) =>
    fc
      .array(grammarErrorGen(textLength), {
        minLength: 1,
        maxLength: maxErrors,
      })
      .map((errors) => {
  ...
  function countVisualIndicators(element: HTMLElement): number {
    ...
    for (const container of Array.from(containers)) {
      const shadowRoot =
        (container as any).shadowRoot || (container as any)._shadowRoot;
      if (shadowRoot) {
        const underlines = shadowRoot.querySelectorAll(
    ...
    const elementShadowRoot =
      (element as any).shadowRoot || (element as any)._shadowRoot;
    if (elementShadowRoot) {
      const underlines = elementShadowRoot.querySelectorAll(
        ".grammar-error-underline",
      );
      count += underlines.length;
    }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (text) => {
          ...
          const errors = await fc.sample(
            nonOverlappingErrorsGen(text.length, 3),
            1,
          )[0];
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (text) => {
          ...
          const errors = await fc.sample(
            nonOverlappingErrorsGen(text.length, 5),
            1,
          )[0];
        ...
        async (text) => {
          ...
          for (const indicator of indicators) {
            const start = parseInt(
              indicator.getAttribute("data-error-start") || "-1",
            );
            const end = parseInt(
              indicator.getAttribute("data-error-end") || "-1",
            );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (text) => {
          ...
          const errors = await fc.sample(
            nonOverlappingErrorsGen(text.length, 10),
            1,
          )[0];
  ...
  it("should create visual indicators consistently for the same errors", async () => {
    const text =
      "This is a test text with some errors in it for testing purposes.";
    const errors: GrammarError[] = [
      {
        start: 10,
        end: 14,
