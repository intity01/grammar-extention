...
describe("Property: Memory Efficiency", () => {
   ...
   */
  async function calculateTotalMemoryUsage(
    languages: Language[],
  ): Promise<number> {
    let totalBytes = 0;
    ...
    for (const language of languages) {
      try {
        const metadata =
          await dictionaryStorage.getDictionaryMetadata(language);
    ...
    await fc.assert(
      fc.asyncProperty(realisticSizeGen, async (sizes) => {
        ...
        if (totalMemory > MAX_MEMORY_BYTES * 0.9) {
          console.log(
            `High memory usage: ${(totalMemory / (1024 * 1024)).toFixed(2)} MB`,
          );
        }
      }),
      { numRuns: 100 },
    ...
    await fc.assert(
      fc.asyncProperty(detectedLanguagesGen, async (detectedLanguages) => {
        ...
        if (detectedLanguages.length < 3) {
          expect(totalMemory).toBeLessThan(
            THAI_SIZE + ENGLISH_SIZE + JAPANESE_SIZE,
          );
        }
      }),
      { numRuns: 100 },
  ...
  it("should verify actual dictionary sizes are within limits", async () => {
    ...
    for (const language of languages) {
      try {
        ...
        console.log(
          `Dictionary for ${language} not available in test environment`,
        );
      }
    }
    ...
    if (totalMemory > 0) {
      expect(totalMemory).toBeLessThanOrEqual(MAX_MEMORY_BYTES);
      console.log(
        `Actual dictionaries total: ${(totalMemory / (1024 * 1024)).toFixed(2)} MB`,
      );
    }
  });
});
