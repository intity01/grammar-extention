...
describe("Property 13: Character-Based Language Detection", () => {
  ...
  const unknownCharGen = fc.oneof(
    fc
      .integer({ min: 0x0030, max: 0x0039 })
      .map((code) => String.fromCharCode(code)), // digits
    fc.constantFrom(
      " ",
      "\t",
      "\n",
  ...
  it("should detect Thai characters using only Unicode range checks", () => {
    fc.assert(
      fc.property(thaiCharGen, (char) => {
        ...
        expect(LanguageDetector.isLanguage(char, Language.JAPANESE)).toBe(
          false,
        );
      }),
      { numRuns: 100 },
    ...
    fc.assert(
      fc.property(englishCharGen, (char) => {
        ...
        const isInRange =
          ...
          (code >= ENGLISH_LOWER_RANGE.start &&
            code <= ENGLISH_LOWER_RANGE.end);
  ...
  it("should detect unknown characters (outside all language ranges)", () => {
    fc.assert(
      fc.property(unknownCharGen, (char) => {
        ...
        const code = char.charCodeAt(0);
        const isInThaiRange =
          code >= THAI_RANGE.start && code <= THAI_RANGE.end;
    ...
    fc.assert(
      fc.property(mixedTextGen, (text) => {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          expect(LanguageDetector.isLanguage(char, expectedLanguage)).toBe(
            true,
          );
        }
      }),
      ...
      fc.property(arbitraryTextGen, (text) => {
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          // Check if character is in English range
            ...
            expect(LanguageDetector.isLanguage(char, Language.ENGLISH)).toBe(
              true,
            );
            if (detectedLanguages.length > 0) {
              expect(detectedLanguages).toContain(Language.ENGLISH);
          ...
          // Otherwise it's unknown
            ...
            expect(LanguageDetector.isLanguage(char, Language.UNKNOWN)).toBe(
              true,
            );
          }
        }
...
describe("Property 12: Mixed Language Segmentation", () => {
  ...
  it("should create new segment when language changes", () => {
    ...
    const languageChangeTextGen = fc
      .tuple(
  ...
  it("should handle single-language text as one segment", () => {
    ...
    fc.assert(
      fc.property(
        fc
          .array(thaiCharGen, { minLength: 1, maxLength: 50 })
          .map((chars) => chars.join("")),
        (text) => {
          const segments = LanguageDetector.segment(text);
    ...
    fc.assert(
      fc.property(
        fc
          .array(englishCharGen, { minLength: 1, maxLength: 50 })
          .map((chars) => chars.join("")),
        (text) => {
          const segments = LanguageDetector.segment(text);
          expect(segments.length).toBe(1);
    ...
    fc.assert(
      fc.property(
        fc
          .array(japaneseCharGen, { minLength: 1, maxLength: 50 })
          .map((chars) => chars.join("")),
        (text) => {
          const segments = LanguageDetector.segment(text);
          expect(segments.length).toBe(1);
